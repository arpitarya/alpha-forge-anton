...
// Re-export the shared client for existing consumers (e.g. useAuthStore).
// Centralizes auth header, 401-refresh, and ApiError normalization in one place.
export { api };
...
async function verifyModeSignature(key: LoginKeyResponse): Promise<boolean> {
  ...
  return crypto.subtle.verify(
    { name: "RSA-PSS", saltLength: PSS_SALT },
    cryptoKey,
    sigBytes,
    canonical,
  );
...
async function getLoginKey(): Promise<LoginKeyResponse> {
  ...
  const res = await api.get<LoginKeyResponse>("/iam/login-key", { skipAuthRefresh: true });
  const key = res.data;
...
export async function loginUser(credentials: LoginRequest): Promise<TokenResponse> {
  ...
  if (keyInfo.mode === "prod") {
    ...
    return api
      .post<TokenResponse>("/iam/login", { encrypted_payload }, { skipAuthRefresh: true })
      .then((r) => r.data);
  }

  // dev: send plain — credentials are visible in DevTools (intentional)
  return api
    .post<TokenResponse>("/iam/login", credentials, { skipAuthRefresh: true })
    .then((r) => r.data);
}

// skipAuthRefresh: a 401 here means the refresh token itself is invalid;
// re-triggering the refresh dance would recurse. Caller clears auth instead.
export const refreshTokens = (refresh_token: string) =>
  api
    .post<TokenResponse>("/iam/refresh", { refresh_token }, { skipAuthRefresh: true })
    .then((r) => r.data);
...
export const deleteApiKey = (id: string) => api.delete(`/iam/api-keys/${id}`).then((r) => r.data);

export const listSessions = () => api.get<SessionResponse[]>("/iam/sessions").then((r) => r.data);
...
export const revokeSession = (id: string) => api.delete(`/iam/sessions/${id}`).then((r) => r.data);

export const logoutAll = () => api.post("/iam/logout-all").then((r) => r.data);
