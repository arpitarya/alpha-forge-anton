...
gold / SGB / equity holdings (quantity / buy_avg / ltp / isin) — that is the
payload that drives `_HOLDINGS_URL_NEEDLE` and `normalize()` in
tickertape_source_helper.
"""
...
HOLDINGS_PAGE = "https://www.tickertape.in/portfolio"
NEEDLES = (
    "holding", "portfolio", "position", "asset",
    "stocks", "etf", "gold", "sgb", "bond", "investment",
)
...
async def main() -> None:
    ...
    try:
        page = await find_or_open_page(browser, HOLDINGS_PAGE, "tickertape")
        if "login" in page.url.lower():
            print("Not logged in — please log in to tickertape.in in Chrome first.")
            return
        captured = await _capture(page, "portfolio")
        ...
        if not captured:
            print(
                "Nothing captured. Try clicking through Gold / Stocks / SGB tabs "
                "inside the portfolio page and re-run, or broaden NEEDLES."
            )
    finally:
        await browser.close()
        await pw.stop()
