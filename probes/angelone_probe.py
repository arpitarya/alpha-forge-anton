"""Attach to existing CDP Chrome, intercept Angel One portfolio + funds XHRs.

Run while logged in to angelone.in in the AlphaForge Anton Chrome:
    uv run python probes/angelone_probe.py

Prints a shape summary of every matching XHR. Look for a response with
`tradingsymbol`, `isin`, `quantity`, `averageprice`, or `ltp` — that is the
holdings payload that drives `_HOLDINGS_URL_NEEDLES` and `normalize()` in
angelone_source_helper. Also walks the funds page to find the cash field.
"""
...
async def main() -> None:
    ...
    try:
        # ── Holdings page ─────────────────────────────────────
        page = await find_or_open_page(browser, HOLDINGS_PAGE, "angelone")
        if "login" in page.url.lower():
            print("Not logged in — please log in to angelone.in in Chrome first.")
            return
        if "/portfolio/holdings" not in page.url:
            try:
                await page.goto(HOLDINGS_PAGE, wait_until="domcontentloaded", timeout=20000)
            except Exception as e:  # noqa: BLE001
                print(f"nav to holdings warning: {e}")
        captured_all.extend(await _capture(page, "holdings"))

        # ── Funds page (cash) ─────────────────────────────────
        try:
            await page.goto(FUNDS_PAGE, wait_until="domcontentloaded", timeout=20000)
            captured_all.extend(await _capture(page, "funds"))
        except Exception as e:  # noqa: BLE001
            print(f"nav to funds warning: {e}")

        print(f"\n{'=' * 60}")
        print(f"Captured {len(captured_all)} candidate requests:")
        print(f"{'=' * 60}\n")
        for c in captured_all:
            print(json.dumps(c, indent=2, default=str))
            print("-" * 60)
        if not captured_all:
            print(
                "Nothing captured. Either the pages need interaction to trigger "
                "the XHR, or the URLs don't contain any of the NEEDLES. Try "
                "scrolling / clicking inside each tab, then re-run."
            )
    finally:
        await browser.close()
        await pw.stop()
