...
## Corpus Check
- 354 files · ~1,973,779 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2137 nodes · 3449 edges · 75 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 1222 edges (avg confidence: 0.74)
...
Cohesion: 0.02
Nodes (149): _pick_cash(), Angel One — capture free-cash balance via CDP from the funds page.  Probe-confir, _to_float(), AngelOneSource, _capture_holdings_via_reload(), _ensure_logged_in(), _extract_equity_holdings(), fetch_holdings_via_browser() (+141 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (67): ABC, default_model(), health(), NewsSource, ProviderAdapter, ProviderHealth, NewsSource abstract base class — the contract every source must implement., The provider's default model id — the first entry in `providers.json`. (+59 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (114): capture_angelone_cash(), main(), Print Angel One free cash via CDP capture of /funds/v2/getRMSLimit.  Attaches to, _capture(), main(), Attach to existing CDP Chrome, intercept Angel One portfolio + funds XHRs.  Run, Compact preview: top-level keys + first list-of-dict path with sample., _shape_summary() (+106 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (94): ChatMessage, ChatRequest, Request/response schemas for the Orff concierge endpoint., Resolved model choice from the frontend ModelPicker.      The frontend already a, Orff concierge service — streams provider tokens as SSE., Async generator yielding SSE-formatted bytes per token snapshot., Async generator yielding SSE-formatted bytes per token snapshot., _redact() (+86 more)
...
Cohesion: 0.04
Nodes (53): HoldingsAggregator, _inr_invested(), _inr_value(), NewsAggregator, NewsAggregator — fans out to all enabled sources in parallel, deduplicates., AllocationSlice, Aggregator response dataclasses + default rebalance targets., RebalanceDrift (+45 more)
...
Cohesion: 0.11
Nodes (28): createApiKey(), encryptCredentials(), extendSession(), getLoginKey(), getMe(), _importPublicKey(), listApiKeys(), listSessions() (+20 more)

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (26): build_system(), Build the system prompt that constrains Orff to the Fux component vocabulary., _vocab(), ComposeRequest, ComposeResponse, Schemas for Orff on-the-fly component composition (§18.3)., A validated UISpec (or the violations that rejected it). `spec` is a tree of, compose() (+18 more)
...
Cohesion: 0.17
Nodes (12): _forward(), iam_proxy(), IAM proxy — forwards all /iam/* requests to Wagner., _capture_holdings_via_reload(), _ensure_logged_in(), _extract_holdings(), fetch_holdings_via_browser(), normalize() (+4 more)
...
Cohesion: 1.0
Nodes (1): Sync stale sources concurrently; cap at 8 s so the request never hangs.
...
Cohesion: 1.0
Nodes (1): Hard refresh: bust all CSV caches, re-sync every source + all wallets.
...
## Knowledge Gaps
- **452 isolated node(s):** `Portfolio filter probe — verifies asset-class chips, sort, PnL filter, and text`, `Attach to existing CDP Chrome, intercept Gullak dashboard XHRs.  Run while logge`, `Compact preview: top-level keys + first list-of-dict path with sample.`, `Capture screenshots of the terminal, portfolio, and preferences pages.  Attaches`, `Get a token from the API and stash it in localStorage so AuthGuard lets us in.` (+447 more)
...
- **Thin community `Community 63`** (3 nodes): `TerminalTopBar.tsx`, `isActive()`, `TerminalTopBar()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 92`** (2 nodes): `Shared slowapi rate limiter instance.`, `limiter.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 93`** (2 nodes): `broker_urls.py`, `CDP trigger-page URLs and XHR needle strings for every broker adapter.  Import w`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 94`** (2 nodes): `Deterministic seed data for the terminal dashboard panels.  Replaced by real bro`, `dashboard_seed.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 95`** (2 nodes): `Indian RSS feed registry — adding a new outlet = one dict entry here, no other c`, `rss_feeds.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 108`** (2 nodes): `Anton app icon (rounded square, ascending orange bars + upward arrow)`, `Anton horizontal logo (ALPHA FORGE ANTON Trading Terminal, ascending bar+arrow mark)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 158`** (1 nodes): `Strip query-string from URL for dedup purposes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (1 nodes): `Inject dev JWT + bypass boot screen, navigate to root.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 180`** (1 nodes): `Return True when the chat rail aside has pointer-events: auto (i.e. actually ope`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (1 nodes): `Open the chat rail if not already interactive.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (1 nodes): `Raised when a paid provider is invoked without user confirmation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (1 nodes): `Singleton guard; tracks which providers incur real cost.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (1 nodes): `Resolved model choice from the frontend ModelPicker.      The frontend already a`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (1 nodes): `Sync stale sources concurrently; cap at 8 s so the request never hangs.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (1 nodes): `Current FX rates used to roll up cross-currency totals (1-hour CSV cache).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (1 nodes): `Hard refresh: bust all CSV caches, re-sync every source + all wallets.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (1 nodes): `RateLimiter (token-bucket)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (1 nodes): `Cloud-Only Inference / Offline Behavior`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (1 nodes): `Anton brand symbol — ascending bar chart with upward arrow (orange gradient, flat style, 2x)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **What is the exact relationship between `BaseBroker / BrokerSource abstraction` and `BaseBroker / BrokerSource abstraction`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `get()` connect `Community 0` to `Community 1`, `Community 2`, `Community 3`, `Community 6`, `Community 39`, `Community 8`, `Community 9`, `Community 10`, `Community 11`, `Community 12`, `Community 13`, `Community 14`, `Community 17`, `Community 18`, `Community 19`, `Community 20`, `Community 22`, `Community 28`?**
  _High betweenness centrality (0.161) - this node is a cross-community bridge._
- **Why does `Text()` connect `Community 12` to `Community 11`, `Community 13`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 128 inferred relationships involving `get()` (e.g. with `_fetch_holdings()` and `run()`) actually correct?**
  _`get()` has 128 INFERRED edges - model-reasoned connections that need verification._
- **Are the 97 inferred relationships involving `str` (e.g. with `run()` and `_shape_summary()`) actually correct?**
  _`str` has 97 INFERRED edges - model-reasoned connections that need verification._
- **Are the 22 inferred relationships involving `alphaforge-logger — Structured rotating-file + console logger.` (e.g. with `CostGuard` and `CostGuardError`) actually correct?**
  _`alphaforge-logger — Structured rotating-file + console logger.` has 22 INFERRED edges - model-reasoned connections that need verification._