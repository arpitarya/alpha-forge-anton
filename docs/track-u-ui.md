...
| **Flow** (process-flow cockpit) | `/flow` | `modules/flow/` — the locked 8-stage spine (Idea→…→Watch) as a view over one edge, with **REAL** per-stage status; UI edge authoring + pre-registration to elgar. Stages beyond Test render honest-pending. See [docs/flow-cockpit.md](flow-cockpit.md). |
| **Nav** | `TerminalTopBar` | Additive: Terminal · Portfolio · **Flow** · **Goals** · **Decisions**. Terminal stays the landing page; Portfolio & Terminal untouched. (Wireframe intentionally omitted.) |
