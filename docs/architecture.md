...
│   │   ├── plans/       plan store plane — plan_loader (reads plans **through the elgar API**, `elgar_bridge`), plan_drift (targets × live actuals, %-only), /plans + /plans/drift routes. **Anton owns no elgar filesystem path**: the elgar sibling tool (~/my_programs/elgar) maintains the store + path, and every read/write goes through `elgar_bridge` (a bad/unreachable store raises `ElgarStoreError` — never a silent local fallback; `just probe elgar-store-guard`). Money docs live in elgar, never in this repo — `fux why plan-store` · ADR `anton-delegates-elgar-store`
...
│   │   ├── edges/       edge-discovery engine — pre-registered hypotheses through gates 1–2 + journal (elgar), trial_ledger (append-only counts-only trial-budget integrity) + null_selftest (random-data trust check). **EB-0 cross-sectional factor edge**: factor_{schema,panel,rank,universe,quality,trend,exits,rebalance} (12-1 momentum + per-rebalance liquidity universe + quality + NIFTY-trend → weekly net-return series) → funnel (+ funnel_report) (Gates 1-3: cscv_pbo + deflated_sharpe + harvey_liu + factor_walkforward + gate3_montecarlo + scenario_library) → signed contracts.TestReport; eb0_cli = `just eb0` (synthetic), eb0_real_cli = `just eb0-real` (the nse-bhavcopy base-rate verdict: quality disabled-pending, result journaled to elgar — never committed). Deterministic, offline, $0. See docs/edges.md
...
│   │   ├── marketdata/  Gate-0 data integrity — NSE bhavcopy ingest (reuses dump_utils I/O, own OHLCV columns incl. ₹ turnover) + point-in-time universe + gate0_integrity (rejects look-ahead / survivorship leakage). One-time networked ingestion helper: bhavcopy_{fetch,service,cli} (`just ingest-nse`, cm-bhav + UDiFF + NIFTY, $0 HTTP — not metered, survivorship-safe, idempotent) + bhavcopy_parse + bhavcopy_manifest → panel_build (`just build-panel`, offline/$0) + panel_universe emit the survivorship-safe liquidity superset (closes ffill + turnover 0-fill) with per-rebalance Gate-0. See docs/edges.md
...
- `backend/app/modules/marketdata/bhavcopy_cli.py` — `just ingest-nse FROM TO`: one-time networked NSE EOD ingestion (cm-bhav + UDiFF via `bhavcopy_fetch.py`/`bhavcopy_service.py`, pure normalization in `bhavcopy_parse.py`, provenance in `bhavcopy_manifest.py`), $0 HTTP (not metered), survivorship-safe, idempotent. `panel_build.py` (`just build-panel`, offline/$0) pins the top-250 liquidity universe (`panel_utils.py`) into the committed `factor_panel.Panel` and runs Gate-0. Probe: `just probe nse-ingest`
...
- `backend/app/modules/flow/` — the **process-flow cockpit** (`/flow`): a view over edge state mapping the locked 8 stages (Idea→…→Watch) to artifacts. `flow_stages.py` derives deterministic per-stage status from `EdgeSpec` + the journal; `flow_author.py` authors + **server-stamps** a pre-registered `EdgeSpec` to elgar (frozen on first run); `flow_service.py` lists/loads edges (journal ∪ UI-authored specs). **Test/Range:** `flow_run.py` invokes the funnel whole against the real panel (byte-identical to the CLI run) in a worker thread; `flow_jobs.py` is the in-memory async job registry (POST starts, poll `/flow/runs/{id}`, no double-run); the Range cone is the funnel's `Cone` shown downside-first. **Plan:** `flow_sizing.py` is the deterministic sizer (fixed-risk + downside cap + ADV cap + fractional-Kelly; the binding minimum wins; `POST /flow/sizing`) — shown, never auto-applied, never an order. **Red-team (the ONLY LLM stage):** `flow_redteam.py` calls `create_gateway().complete()` (cage-metered by construction), parsing a two-tier critique (severity-tagged objections + 10th-Man dissent + runner-ups + tripwires) — OFF the deterministic path (imports no funnel/sizing engine), cached per edge, background job. **Approve:** `flow_approve.py` assembles a downside-first `ApprovalProposal` + exec checklist and records a binary, **ack-loss-first**, PII-guarded (`critic_guard.pii_block`) decision **journaled to elgar** (`flow_decision_store.py`, fail-loud) with a logged cooldown — **never places an order**. **Live:** `flow_live.py` PREPARES copy-only orders (entry + staged −12/−20 guard + checklist) for an approved edge and RECONCILES human-entered fills into true P&L + slippage — it **imports no broker module and makes no order-placement call** (the hard invariant; reconciliation is manual-fill-entry, no live-auth). **Watch:** `flow_watch.py` is the deterministic decay monitor (`analyze` → severity-tagged decay signals + verdict; `decay-kill` journals a `RetirementRecord` to elgar without mutating the frozen spec). **All 8 process-flow stages ship** (Idea→…→Watch), edges authored + decided in the UI; Red-team is the only LLM stage, Live never places an order. Probes: `flow-author` · `flow-run` · `flow-sizing` · `flow-redteam` · `flow-approve` · `flow-live` · `flow-watch` · `ui-flow`. See [flow-cockpit.md](flow-cockpit.md)
