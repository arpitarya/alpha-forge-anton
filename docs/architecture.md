...
- `backend/app/modules/concierge/` — Orff concierge backend, wired to the **Fux brain** on both paths (the §18 vision: Fux serves Claude Code at dev-time and Orff at runtime):
  ...
  - `history_service.py` — conversation history, **one elgar doc per session** (`orff-session-<id>`): `save_session`/`load_session`/`list_sessions`/`delete_session`. The doc is a human-readable transcript plus a machine block for lossless resume; the store's git log is the conversation's audit trail. Best-effort, like memory
