...
- `backend/app/modules/concierge/` — Orff concierge backend, wired to the **Fux brain** on both paths (the §18 vision: Fux serves Claude Code at dev-time and Orff at runtime):
  ...
  - `concierge_routes.py` / `concierge_schemas.py` — SSE chat, `POST /concierge/compose`, and `GET/PUT /concierge/memory` endpoints; `ChatMessage.images` carries vision attachments (data URLs, capped); `stt_service.py` — speech-to-text for the voice bar
