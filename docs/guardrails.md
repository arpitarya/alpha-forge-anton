...
- **`fux gate` checks rule integrity, not file content — so the `gate` job also runs `plan_safety_probe.py`.** `fux gate` blocks on tampered/unsealed/unratified constitutional *rules*; it does **not** scan committed files for money docs or PII. A `*.plan.md` (or a PAN / account number) committed with `--no-verify` (bypassing the local `dante pii` hook) therefore passes `fux gate`. The `gate` CI job closes this for the **money-doc class** by running the pure-stdlib `probes/plan_safety_probe.py` (CI twin of `just probe plan-safety`): a tracked `*.plan.md`/`*.drift.md` or a leaking rule doc fails the required check. **Residual gap (documented, not closed):** a stray *hard identifier inside a non-plan file* is caught today only by `dante pii` in pre-commit — `dante` is a local sibling tool (`alphaforge-dante`, path dep), not a pip package, so it cannot run in CI without being published/vendored. Until then that one PII-content dimension is a bypassable-by-`--no-verify` local guard, not a wall. Closing it = publish/vendor dante (or port its PII patterns into a stdlib probe) and add it to the `gate` job.
